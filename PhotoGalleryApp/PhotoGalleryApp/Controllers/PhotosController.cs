...
namespace PhotoGalleryApp.Controllers
{
    ...
    .FirstOrDefaultAsync(m => m.PhotoId == id);
        ...
        public async Task<IActionResult> ToggleLike(int id)
        {
            try
            {
                var photo = await _context.Photos.Include(p => p.Likes).FirstOrDefaultAsync(p => p.PhotoId == id);
                if (photo == null)
                {
                    return NotFound();
                }
                ...
                var like = await _context.Likes.FirstOrDefaultAsync(l => l.PhotoId == id && l.UserId == userId);

                if (like == null)
                {
                    _context.Likes.Add(new Like { PhotoId = id, UserId = userId });
                }
                else
                {
                ...
                var updatedPhoto = await _context.Photos.Include(p => p.Likes).FirstOrDefaultAsync(p => p.PhotoId == id);
                return Ok(updatedPhoto.Likes.Count);
            }
            catch (Exception ex)
            {
